...
def create_images(scene,params,parameter_changed):
    ...
    for j in range(11):
        ...
        params.update()
        sensorIndex=0
        image= scene.integrator().render(scene,seed=1, sensor_index=sensorIndex);
        ...
        film = sensor.film();
        img= film.bitmap();
        img = img.convert(Bitmap.PixelFormat.RGB, Struct.Type.UInt8, srgb_gamma=True);
...
# roughness
params['Material.base_color.value'] =ScalarColor3f(np.array([0.1,0.3,0.2]))
params['Material.base_color.value'] =ScalarColor3f([46/256,47/256,99/256])
...
#specular
params['Material.base_color.value'] =torch.Tensor([250,10,10]) / 256
params['Material.roughness.value'] = 0.01
params['Material.anisotropic.value'] = 0.0
...
images_specular_tint=create_images(scene,params,"specular_tint.value")
...
images_st_spec_tint = create_images(scene,params,"specular_tint.value")
...
images_st_clearcoat_gloss = create_images(scene,params,"clearcoat_gloss.value")
...
images_st_sheen = create_images(scene,params,"sheen_tint.value")
...
pad=1
parameters = ["roughness","metallic","specular","specular_tint","anisotropic","clearcoat","clearcoat_gloss","sheen","sheen_tint","flatness"]
...
for j in range(11):
    axs[j, 0].annotate(str(j/10),xy=(0, 0.5), xytext=(-axs[j, 0].yaxis.labelpad - pad, 0),
                xycoords=axs[j, 0].yaxis.label, textcoords='offset points',
                size='large', ha='right', va='center',fontweight='semibold')
...
images_st.append(images_st_specular_tint)
...
parameters = ["roughness","metallic","specular","specular_tint","anisotropic","clearcoat","clearcoat_gloss","sheen","sheen_tint","spec_trans"]
fig, axs = plt.subplots(12, len(parameters),figsize=(10,12),dpi=300);
...
for i in range(12):
    for j in range(10):
        ...
        axs[i,j].set_xticks([])
plt.subplots_adjust(wspace=0, hspace=0)
plt.savefig("disney_st_blend.jpg")
