...
# 1st scene for thin
scene = load_file('scene_thin.xml')
...
# 2nd scene for thin
scene = load_file('scene_thin_light.xml')
params = traverse(scene);
...
pad=1
parameters = ["spec_trans","roughness","eta(1.0-2.0)","anisotropic","diff_trans(0.0-2.0)"]
...
for j in range(len(parameters)):
    axs[j, 0].annotate(parameters[j],xy=(0, 5), xytext=(-axs[j, 0].yaxis.labelpad - pad, 0),
                xycoords=axs[j, 0].yaxis.label, textcoords='offset points',
                size='small', ha='right', va='center',fontweight='semibold')
...
plt.savefig("thin_blend.png",bbox_inches='tight', pad_inches=0.01)
